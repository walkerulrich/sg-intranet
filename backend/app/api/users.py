"""
Route /api/users — liste & recherche d'employés.
⚠ Contient des failles SQLi pour le lab.
"""
...
async def list_users(db: AsyncSession = Depends(get_db)):
    """Liste tous les utilisateurs (route propre, paramètres liés via ORM)."""
    result = await db.execute(select(User).order_by(User.id))
    return list(result.scalars().all())


# ─── FAILLE : Recherche d'employé vulnérable à la SQLi ──────────────
# Payload Burp : ?q=' OR '1'='1
@router.get("/search")
...
# ─── FAILLE : Détails d'un utilisateur par ID, vulnérable à la SQLi ─
# Payload Burp : /api/users/1 UNION SELECT username,password,... (via PATH)
@router.get("/by-name/{name}")
async def get_user_by_name(name: str, db: AsyncSession = Depends(get_db)):
    """
    Récupère un utilisateur par son nom complet.
    ⚠ VULNÉRABLE : input non échappé dans la requête.
    PAYLOAD : /api/users/by-name/anything' UNION SELECT username,password,null,null,null,null,null,null,null FROM users--
    """
